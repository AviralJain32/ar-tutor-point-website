      ...
      <div className=" font-Main-font pb-2 justify-between bg-white items-center md:pb-0 md:flex ">
        ...
        >
          {navLinks.map(navLinks => (
            <li
              key={navLinks.Name}
              className="my-7 font-medium text-center text-[14px] px-4 md:my-0 md:text-md md:px-1 hover:text-gray-400 duration-200 ease-in-out"
            >
        ...
        <div className=" flex items-center justify-center hover:scale-105 duration-300 ease-in-out">
          <button
            className={`hover:scale-1.70 animate-pulse text-white font-bold text-[14px] px-4 py-2 md:text-sm bg-primary rounded-md md:ml-0 ${open ? '' : 'hidden'} md:block`}
          >
            Register A Call
          </button>
        </div>
